...
class SlideDataset(Dataset):
	def __init__(self, dataset_name, csv_path, features_path, modalities, embedding_size=None, sample=-1, cohort=None, train=True):
		"""
		Args:
		...
		self.sample = sample    # bag_level 采样次数
	...
	def __getitem__(self, index):
		...
		if self.train:
			...
			all_feats = []
			for modality, modality_label in zip(self.modalities, modality_labels):  # modality_labels表明是否存在某一模态
				# curr_h5_path = os.path.join(self.features_path, f"{slide_id}_{modality}{special_id}.h5")
				if modality == 'HE':
					# features_path = '/data2/jsh/ZJU/data_processed/Origin'
					# features_path = '/ZJU/data2/'
					self.cohort = '256'
					type = 'UNI2'
					curr_h5_path = os.path.join(self.features_path, 'HE_{}'.format(self.cohort), type, 'h5_files', f"{slide_id}.h5")
					curr_pt_path = os.path.join(self.features_path, 'HE_{}'.format(self.cohort), type, 'pt_files', f"{slide_id}.pt")
					# curr_h5_path = '/data2/jsh/ZJU/data_processed/Registration/HE_224/CONCH/h5_files/201419275.3.ome.h5'
					# curr_pt_path = '/data2/jsh/ZJU/data_processed/Registration/HE_224/CONCH/pt_files/201419275.3.ome.pt'
				else:
					# features_path = '/data2/jsh/ZJU/data_processed/Registration'
					# features_path = '/ZJU/data2/'
					self.cohort = '256'
					# curr_h5_path = '/data2/jsh/ZJU/data_processed/Registration/IF_224/CONCH/CD3/h5_files/201419275.3.ome.h5'
					# curr_pt_path = '/data2/jsh/ZJU/data_processed/Registration/IF_224/CONCH/CD3/pt_files/201419275.3.ome.pt'
					type = 'UNI2'
					curr_h5_path = os.path.join(self.features_path, 'IF_{}'.format(self.cohort), type, modality, 'h5_files', f"{slide_id}.h5")
					curr_pt_path = os.path.join(self.features_path, 'IF_{}'.format(self.cohort), type, modality, 'pt_files', f"{slide_id}.pt")
				# curr_feats = load_features(curr_h5_path) if modality_label == 1 else torch.zeros([2, self.embedding_size])
				curr_feats = torch.load(curr_pt_path) if modality_label == 1 else torch.zeros([2, self.embedding_size])
...
def collate(batch):
		...
		try:
			...
			print(str(slide_ids) + ' error')
			return {
            "feats": torch.empty(0),
            "modality_labels": torch.empty(0),
            "slide_ids": slide_ids
            }

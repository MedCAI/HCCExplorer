...
class MLDataset_Graph_Recur(Dataset):

    def __init__(self, model_name='mil', csv_path = 'label.csv', data_dir=None, shuffle = False, seed = 7, n_bins = 4, recur_label_col = None, eps=1e-6):
        super(MLDataset_Graph_Recur, self).__init__()
        ...
        columns_to_keep = ['id', 'recurrent', 'living_status', 'OS', 'DFS', 'folder', 'file_name']
        new_df = slide_data[columns_to_keep]
        wsi_id_list = [id.split('.svs')[0] for id in new_df['file_name'].to_list()]
        new_df.insert(1, 'wsi_id', wsi_id_list)

        slide_data = new_df
        patients_df = slide_data.copy()
        recur_uncensored_df = patients_df[patients_df['recurrent']==1]
        ...
        for i in slide_data.index:
            ...
            censorship = slide_data.loc[i, 'recurrent']
    ...
    def __getitem__(self, idx):
        ...
        e = self.slide_data['recurrent'][idx]
        ...
        data_re = Data(x=data_origin.x[:,:512], edge_index=data_origin.edge_index)
        mock_data = Data(x=data_origin.x[:,:512], edge_index=data_origin.edge_index, pos=data_origin.pos)        
...
class Generic_Split_Graph(MLDataset_Graph_Recur):
    def __init__(self, model_name, slide_data, recur_label_col='survival_month', data_dir=None):
        self.model_name = model_name
        self.use_h5 = False
        self.slide_data = slide_data
        self.recur_label_col = recur_label_col

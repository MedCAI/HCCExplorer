...
def make_weights_for_balanced_classes_split(task, dataset):
    if task == 'recur':
        N = float(len(dataset))                                               
        weight_per_recur_class = [N/len(dataset.slide_recur_ids[c]) for c in range(len(dataset.slide_recur_ids))]                                                                                                  
        weight = [0] * int(N)                                           
        for idx in range(len(dataset)):   
            (surv_y) = dataset.getlabel(idx)                        
            weight[idx] = weight_per_recur_class[surv_y]                            
